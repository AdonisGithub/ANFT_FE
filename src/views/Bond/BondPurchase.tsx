...
function BondPurchase({ bond }: IBondPurchaseProps) {
    ...
    async function onBond() {
        ...
        if (quantity === "") {
            ...
            if (shouldProceed) {
                const trimBalance = trim(Number(quantity), 6);
    ...
    const setMax = () => {
        ...
        let amount: any = bond.maxBondPriceToken * 0.9999 ;

        if (amount) {
            amount = trim(amount,5);
        }
            ...
            <Slide direction="left" in={true} mountOnEnter unmountOnExit {...{ timeout: 533 }}>
                <Box className="bond-data">
                    ...
                    <div className="data-row">
                        ...
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.bondQuote, 4)} ANFT`}</p>
                    </div>

                    <div className={`data-row`}>
                        <p className="bond-balance-title">Max You Can Buy</p>
                        <p className="price-data bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.maxBondPrice, 4)} ANFT`}</p>
                    </div>
                    ...
                    <div className="data-row">
                        ...
                        <p className="bond-balance-title">{isBondLoading ? <Skeleton width="100px" /> : `${trim(bond.minBondPrice, 4)} ANFT`}</p>
                    </div>
                </Box>
            </Slide>
        </Box>
